...
The four CLI entry points live under [`scripts/workflows/`](scripts/workflows/) — one per analysis style (see [§4](#en-4) for which to pick). Each takes the same kind of inputs and accepts `-h` for the full option list.
...
The bundled demo needs nothing extra. For real runs:

- **TAIR10 reference (~250 MB)** — needed by `promoter.sh` / `elements.sh`. Run `bash scripts/workflows/cli/00_env_check.sh` (also checks `$PATH` tool versions).
- **21-species pre-computed indexes (~16 GB)** — needed by `pair_only.sh` against canonical species and by the web app's `promoters_pre` mode. Run `make fetch-data` **once**.

R heatmap is best-effort: install per [`scripts/r/install_packages.R`](scripts/r/install_packages.R) or skip — `motif_output.txt` is produced regardless.
...
| `deploy/configure/email_credential.txt` | so users get a result-link email when their task finishes; without it task completion still works, just no notification | 5 lines: `smtp_login` / `smtp_key` / `from_address` / `smtp_server` / `port`. Default setup uses Brevo (EEA-hosted, no Gmail dependency); see [§9.7](#en-9). Gitignored — **never commit it**. |
...
- **Admin**: see [§9.6](#en-9) for setup, login, and capabilities.
...
Single shared token grants "see all tasks + terminate any task". Regular users only see tasks matching their email; admins see everything.

**Enable** — easiest path: `make up` detects the missing file on an interactive shell and offers to generate one (`Y/n`). Manual equivalent: `openssl rand -hex 32 > deploy/configure/admin_token.txt`. Without this file, `/admin/*` returns `503`.

**Log in** — open [`/admin/login`](http://localhost:5960/admin/login), paste the token. Cookie lasts 30 days.

**What you can do**:

- See all tasks at `/tasks` (search box still works as a substring filter on email / task ID)
- Filter by status / mode / date (client-side dropdowns over the latest 200 tasks)
- **Terminate** pending or running tasks — backend walks the worker process tree with `psutil` (SIGTERM, then SIGKILL after 5 s), marks the task `cancelled`, emails the user
- **`/admin/settings`** toggles `notify_on_submit` — turns the per-task "New Task Submitted" admin email on/off (user-facing started / completed / cancelled emails unaffected). Persists in `deploy/configure/admin_settings.json`, hot-reloaded.
...
|---|---|---|
| **Outbound** — task notifications | [Brevo](https://www.brevo.com) (France) | Docker container → [DO VPS `socat` :10587](docs/email-setup.md#en-2-3) → `smtp-relay.brevo.com:587`. The relay chain masks the Berlin server's dynamic home IP behind the fixed DO VPS IP so Brevo's IP allowlist only needs one entry (`206.81.24.229`). SMTP credentials live in `deploy/configure/email_credential.txt`. |
...
| CLI baseline ([`tests/baseline/`](tests/baseline/)) | `make baseline-check` (alias: `make baseline`) | Hashes every demo-run output file + production binaries, then compares against the committed `fingerprints.txt`. Substance-only diff (timestamp / git SHA header ignored). **Exits non-0 on real divergence** so CI can gate on it. Captures to gitignored `fingerprints.actual.txt`; workspace stays clean. To intentionally re-bless the baseline after a behavioural change, use `make baseline-update`. ~30 s. |
...
# 2. 在 demo 自带的 homotypic 索引上跑 pairing（~5 秒）。
bash scripts/workflows/pair_only.sh \
    -d data/demos/promoters/pairing/demo \
    -g data/demos/promoters/pairing/demo/gene.txt \
    -o results/cli/demo/pairing
...
4 个 CLI 入口都在 [`scripts/workflows/`](scripts/workflows/) 下（哪种分析挑哪个见 [§4](#cn-4)），输入形式一致，`-h` 看完整选项。
...
- **TAIR10 参考（~250 MB）** —— `promoter.sh` / `elements.sh` 要用。跑 `bash scripts/workflows/cli/00_env_check.sh`（顺带检查 `$PATH` 上的工具版本）。
- **21 物种预计算索引（~16 GB）** —— `pair_only.sh` 跑 canonical 物种、web app 的 `promoters_pre` 模式都要用。`make fetch-data` 跑**一次**。

R heatmap best-effort：按 [`scripts/r/install_packages.R`](scripts/r/install_packages.R) 装包或者跳过 —— `motif_output.txt` 都照出。
...
| `deploy/configure/email_credential.txt` | 任务跑完给用户发结果链接邮件；缺它任务还能跑，只是没通知 | 5 行：`smtp_login` / `smtp_key` / `from_address` / `smtp_server` / `port`。默认用 Brevo（EEA 托管，不依赖 Gmail）；详见 [§9.7](#cn-9)。Gitignored —— **不要提交**。 |
...
一份共享 token 授予"看全部任务 + 终止任意任务"。普通用户只能按邮箱看自己的；管理员看全部。

**启用** —— 最简单：直接 `make up`，交互 shell 下检测到文件缺失会问"是否生成 token？[Y/n]"。手动等价命令：`openssl rand -hex 32 > deploy/configure/admin_token.txt`。缺这文件时 `/admin/*` 直接返回 `503`。

**登录** —— 打开 [`/admin/login`](http://localhost:5960/admin/login) 粘 token。Cookie 30 天。

**能干啥**：

- `/tasks` 看全部任务（搜索框仍是邮箱 / task ID 子串筛）
- 按状态 / mode / 日期筛（客户端下拉，在最新 200 条上过滤）
- **终止** pending / running 任务 —— 后端用 `psutil` 递归 walk worker 进程树（先 SIGTERM，5 秒未死再 SIGKILL），标 `cancelled`、给用户发邮件
- **`/admin/settings`** 切 `notify_on_submit` —— 控制"新任务提交"那封管理员通知开关（用户侧的开始 / 完成 / 被取消邮件不受影响）。开关持久化到 `deploy/configure/admin_settings.json`，hot-reload。
...
| 方向 | 服务 | 机制 |
|---|---|---|
| **发信** —— 任务通知 | [Brevo](https://www.brevo.com)（法国） | Docker 容器 → [DO VPS `socat` :10587](docs/email-setup.md#cn-2-3) → `smtp-relay.brevo.com:587`。这个中继链把 Berlin 服务器的动态家宽 IP 藏在固定的 DO VPS IP 后面，Brevo 白名单只需一条 `206.81.24.229`。SMTP 凭据在 `deploy/configure/email_credential.txt`。 |
